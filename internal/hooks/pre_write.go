...
// Always returns a message â€” warning when impact > 10 nodes, informative otherwise.
...
func PreWriteMessage(q *graph.Querier, filePath string, depth int, rootName string) string {
	...
	if len(impact) > 10 {
		return fmt.Sprintf("[engrafo] WARNING: modifying %s may impact %d files â€” review blast radius",
			filePath, len(impact))
	}
