...
func (s RootSpec) Resolve(manifestDir string) (graph.ResolvedRoot, error) {
	if !validName.MatchString(s.Name) {
		return graph.ResolvedRoot{}, fmt.Errorf("nombre de raíz inválido %q: debe coincidir con ^[a-zA-Z0-9._-]+$", s.Name)
	}
	if s.Path == "" {
		return graph.ResolvedRoot{}, fmt.Errorf("raíz %q: path es obligatorio", s.Name)
	}
	...
	if _, err := os.Stat(absRoot); err != nil {
		return graph.ResolvedRoot{}, fmt.Errorf("raíz %q: el directorio %q no existe", s.Name, absRoot)
	}

	// rel_path: path del manifest (relativo o ".") que apunta a esta raíz.
	relPath := s.Path
	if filepath.IsAbs(relPath) {
		relPath = "."
	}

	// Detectar VCS si no está explícito.
	vcs := s.VCS
	if vcs == "" {
	...
	// Resolver remote desde git si no se especificó.
	remote := s.Remote
	if remote == "" && vcs == "git" {
	...
	// Resolver branch desde git si no se especificó.
	branch := s.Branch
	if branch == "" && vcs == "git" {
...
// Save escribe el manifest en path con indentación de 2 espacios.
...
// validateManifest verifica la integridad del manifest: nombres válidos y únicos, paths presentes.
func validateManifest(m *Manifest) error {
	seen := make(map[string]bool, len(m.Roots))
	for _, r := range m.Roots {
		if !validName.MatchString(r.Name) {
			return fmt.Errorf("nombre de raíz inválido %q: debe coincidir con ^[a-zA-Z0-9._-]+$", r.Name)
		}
		if seen[r.Name] {
			return fmt.Errorf("nombre de raíz duplicado %q", r.Name)
		}
		seen[r.Name] = true
		if r.Path == "" {
			return fmt.Errorf("raíz %q: path es obligatorio", r.Name)
		}
	}
	return nil
}

// FindManifest busca engrafo.json subiendo desde startDir hasta la raíz del filesystem.
// Devuelve la ruta al manifest, el directorio que lo contiene y true si se encontró.
...
// FindGitRoot busca .git subiendo desde startDir hasta la raíz del filesystem.
// Devuelve el directorio que contiene .git y true si se encontró.
